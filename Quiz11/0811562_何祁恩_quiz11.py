...
def TASK1(A:np.ndarray, rs:int):
    ...
    pca = PCA(.95, random_state = rs)
    X = pca.fit_transform(A_scaled)
...
def TASK3(classifier, flag = False):
    ...
    x = np.c_[iris['data'][:,0], iris['data'][:,2]]
    y = iris['target']
...
def TASK4(t_size:float, rs:int, n_estimators:int, lr:float, md:int):
    ...
    gbc = GradientBoostingClassifier(random_state = rs, n_estimators = n_estimators, learning_rate = lr, max_depth = md)
